    ...
    internal static class GzipExtensions
    {
        ...
        public static Stream GetCompressedResponseStream(this HttpWebResponse response)
        {
            ...
            if (string.Equals(response.Headers[HttpRequestHeader.ContentEncoding], GZIP, StringComparison.OrdinalIgnoreCase))
            {
                ...
                var decompressedStream = Application.GetResourceStream(
        ...
        private static void LogCompression(long compressedStreamLength, long decompressedStreamLength)
        {
            ...
            {
                ratio = (double) compressedStreamLength / decompressedStreamLength;
            }

            Debug.WriteLine(string.Format("CompressedLength={0}, DecompressedLength={1}, Ratio={2}",
                compressedStreamLength,
                decompressedStreamLength,
                ratio));
        }
    }
}
